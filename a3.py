...
def make_str_from_column(board, column_index):
    ...
    for i in range(len(board)):
      complete= complete + board[i][column_index]
    return complete
...
def num_words_on_board(board, words):
    ...
    for j in range(len(words)):
     for i in range(len(board)):
          if words[j] in make_str_from_row(board, i):
            count=count+1
     for u in range(len(board[0])):
          if words[j] in make_str_from_column(board, u):
            count=count+1
    return count
...
def read_words(words_file):
    ...
    word_list=[]
    for line in file:
        word_list.append(str((line.rstrip('\n'))))
    file.close()
    return word_list
def read_board(board_file):
    ...
    board_list=[]
    for line in file:
     wordlist=str((line.rstrip('\n')))
     inner=[]
     for i in range(len(wordlist)):
          inner.append(str(line[i]))
     board_list.append(inner)        
    return board_list
